...
export class DataSource extends Disposable {
	 ...
	 */
	private getLog(repo: string, branches: ReadonlyArray<string> | null, num: number, includeTags: boolean, includeRemotes: boolean, includeCommitsMentionedByReflogs: boolean, onlyFollowFirstParent: boolean, order: CommitOrdering, remotes: ReadonlyArray<string>, hideRemotes: ReadonlyArray<string>, stashes: ReadonlyArray<GitStash>, author?: string, committer?: string, commitMessage?: string, commitHash?: string, dateFrom?: number, dateTo?: number, paths?: string[]) {
		...
		if (commitMessage) {
			args.push('--grep=' + commitMessage);
		}
		if (commitHash) {
			args.push(commitHash);
		}
		if (dateFrom) {
			args.push('--after=' + dateFrom);
		}
		if (dateTo) {
			args.push('--before=' + dateTo);
		}
